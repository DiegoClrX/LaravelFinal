    ...
    <div class="py-12">
                        ...
                        <table class="w-full text-md bg-white shadow-md rounded mb-4">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b hover:bg-orange-100 bg-gray-100">
                                    ...
                                    <td class="p-3 px-5" >{{ $restaurante->telefono }}</td>
                                    <td class="p-3 px-5" style="width: 7%"><img src="{{ url('storage/restaurantes/img', [$restaurante->foto]) }}" alt="" width="100%"></td>
                                    <td class="p-3 px-5" >{{ $restaurante->latitud }}</td>
                                    <td class="p-3 px-5" >{{ $restaurante->longitud }}</td>
                                    <td class="p-3 px-5 flex justify-end">

                                        {{-- BOTON PARA ACCEDER A PEDIDOS --}}
                                        {{-- <a href="{{Route('platos.show',$restaurante->id)}}" style="margin-right: 5%"> --}}
                                        <a href="{{Route('restaurante.verPedidos', $restaurante->id)}}" style="margin-right: 5%;">
                                            <button class="inline-block p-3 text-center text-white transition bg-yellow-500 rounded-full shadow ripple hover:shadow-lg hover:bg-yellow-600 focus:outline-none">
                                                <img class="w-5 h-5 text-white" src="{{ asset('img/pedido.png') }}" />
                                            </button>
                                        </a>

                                        {{-- BOTON PARA ACCEDER A PLATOS DEL RESTAURANTE --}}
                                        <a href="/intranet/restaurante/restaurante/{{$restaurante->id}}/platos" style="margin-right: 5%;">
                                            ...
                                            <img class="w-5 h-5 text-white" src="{{ asset('img/platos.png') }}"/>
                                          </button>
                                        </a>
                                            {{-- BOTON PARA ACTUALIZAR EL RESTAURANTE --}}
                                            <a href="{{Route('restaurante.edit', $restaurante->id)}}">
                                            ...
                                            {{-- BOTON PARA ELIMINAR EL RESTAURANTE --}}
                                            <form action="{{Route('restaurante.destroy', $restaurante->id)}}" method="POST" style="margin-left: 5%">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @else
                                    <h1 style="width: 100%; margin-left: 40%">No tienes restaurantes </h1>
                                @endif
        {{-- paginador --}}
        ...
        <form action="{{Route('restaurante.store')}}" method='post' enctype="multipart/form-data">
            ...
            <div class="main-modal fixed w-full h-100 inset-0 z-50 overflow-hidden flex justify-center items-center animated fadeIn faster"
                style="background: rgba(0,0,0,.7); width: 100%; overflow: scroll; margin-top: -15%">
                <div style="width: 100%; height:250px">
                    <div class="w-full mx-auto rounded-lg bg-white shadow-lg p-5 text-gray-700" style="max-width: 600px;">
                        ...
                        <div class="mb-10" style="margin-bottom: -3%">
                            <h1 class="text-center font-bold text-xl uppercase">New Restaurant</h1>
                        </div>
                        <div class="mb-3">
                            <label class="font-bold text-sm mb-2 ml-1">Name of Restaurant</label>
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" type="text" name = 'name'/>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="font-bold text-sm mb-2 ml-1">Address of Restaurant</label>
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" name="address" type="text"/>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="font-bold text-sm mb-2 ml-1">City</label>
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" name="city" type="text"/>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="font-bold text-sm mb-2 ml-1">Telefono</label>
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" name="telefono" type="text"/>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" name="latitud" type="text"/>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="font-bold text-sm mb-2 ml-1">Longitud</label>
                            <div>
                                <input class="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors" name="longitud" type="text"/>
                            </div>
                        </div>
                                <!--Footer-->
